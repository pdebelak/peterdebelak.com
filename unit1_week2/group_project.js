//Create user_nums array
var user_nums = [];
//Using loop-
//Request number from user
//Exit loop when user is done entering numbers
var input = "";
do {
    input = prompt("Please enter your numbers. Leave blank and click OK to finish.");
    if (input === "" || input === null) {
        break;
    } else {
        user_nums.push(parseFloat(input)); //Push number onto array
    }
} while (1);
...
function sum_array() {
  ...
  alert("Sum is " + runningSum);
}
 
//Create mean_of_array
function mean_of_array () {
  var runningSum = 0;
  for (var i = 0; i < user_nums.length; i++) {
    runningSum = runningSum + user_nums[i];
  }
